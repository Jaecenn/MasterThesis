...
{\left(\frac{L/4}{L/2}\right)}^{\frac{1}{N}}={\left(\frac{1}{2}\right)}^{\frac{1}{N}}=0.8706
\end{equation}
With the coefficient $q$ we can simply put the corresponding numbers to the equation \ref{eq:spectro1} for $n=0\ldots N$. The results are border values (as frequency bins) of all the 5 bins and they are shown in table \ref{tab:indiciesBins}.
...
The summation is done over the ranges noted in table \ref{tab:indiciesBins}. After all the values in given range are summed an effective value of the spectrum (RMS) is calculated. This is described in \cite{cite:1} and the equation for obtain RMS value of the spectrum is  
\begin{equation} \label{eq:spectro3}
RMS_k=\frac{\sqrt{2}}{L}\cdot \sqrt{|X_k|^2},
\end{equation}
where $k=1\ldots \frac{L}{2}-1$ and $k=0$.
